...
%% MAIN FUNCTION 
    ...
    glob_verb=verb;
    for i=1:last_image
        if  verb>=0 && isempty(intersect(i,indices))
            verb=0;
        else
            verb=glob_verb;
...
function [image]=obtain_image(fid,A,image_size,block_size,nb_frames,verb)
    ...
    if any(strfind(A,zeros([1 block_size]))) % Detect possible WTF block of block_size null elements.
        n_zeros=block_size;
        while any(strfind(A,zeros([1 n_zeros+block_size])))
            n_zeros=n_zeros+block_size;
        end
        A=[A fread(fid,n_zeros,'uint16=>uint16','l')'];
        idx=strfind(A,zeros([1 n_zeros])); % in the image
        A=[A(1:idx-1) A(idx+n_zeros:end)]; % and bypass it
    end
